...
def show(id):
    ...
    image = Image.query.filter_by(item_id = id).all()
    ...
    return render_template("item/show.html", item = item, image = image, user=user, bids = bids, formatted_datetime = formatted_datetime, bidders = bidders)#,item=item, form=form, etc)
